...
import { Chat, Chatbot, IChat, Message, User } from "../models/User.js";
...
export const getAllGurus = async (
  ...
  try {
    ...
    const chatbotDetails = chatbots.map((chatbot) => {
      ...
      const chatIds = (chatbot.chats as unknown as IChat[]).map((chat: IChat) => chat._id);  // Extract _id from each populated IChat
      console.log(chatIds)
      return {
        id: chatbot._id,
        name: chatbot.name,
...
export const getAllChatsOfGuru = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const getChatOfGuru = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const createDefaultGuru = async (
  userId: string
): Promise<{ chatbotId: string; chatId: string; navigateurl: string }> => {
  try {
