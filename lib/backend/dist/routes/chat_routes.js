...
import { chatCompletionValidator, chatIdValidator, userIdValidator, validate, } from "../utils/validators.js";
import { verifyToken } from "../utils/token_manager.js";
import { clearChatMessages, deleteChatSession, deleteAllChatSessions, generateChatCompletion, sendChatsToUser, getUserChats, createOrGetChatSession, getChatSession, updateChatTitleController, generateChatTitle, } from "../controllers/chat_controllers.js";
const chatRoutes = Router();
// Route for starting a new chat session
chatRoutes.post("/start", 
...
chatRoutes.get("/users/:userId", verifyToken, validate(userIdValidator), getUserChats);
chatRoutes.get("/:chatId/:userId", 
// verifyToken,
getChatSession);
chatRoutes.put('/u-t', updateChatTitleController);
// Route for retrieving all chats for the authenticated user
chatRoutes.get("/all", sendChatsToUser);
// Route for clearing messages in a specific chat
chatRoutes.post("/:chatId/clear", validate(chatIdValidator), clearChatMessages);
...
chatRoutes.delete("/:chatId", validate(chatIdValidator), deleteChatSession);
// Route for deleting all chats for the authenticated user
chatRoutes.delete("/all", deleteAllChatSessions);
