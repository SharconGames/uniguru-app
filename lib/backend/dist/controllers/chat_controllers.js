...
import { User, Chat, Message } from '../models/User.js';
import { summarizeHistory } from '../utils/summary.js';
import mongoose from 'mongoose';
// Helper function to validate ObjectId
const isValidObjectId = (id) => mongoose.Types.ObjectId.isValid(id);
const generateAIResponse = async (context, message, chatbot, model) => {
...
export const generateChatTitle = async (chatId, userId, message, model, chatbotId) => {
    try {
...
export const createOrGetChatSession = async ({ userId, chatbotId, createNewSession, }) => {
...
export const generateChatCompletion = async (req, res) => {
    ...
    try {
        ...
        const aiResponse = await generateAIResponse('', // Empty context if not needed
        message, chatbot, model || 'llama3-8b-8192');
...
export const sendChatsToUser = async (req, res, next) => {
    try {
        ...
        const groupedChats = user.chats.reduce((acc, chat) => {
            // Check if chatbot is populated and exists
            if (!chat.chatbot || !chat.chatbot._id)
                return acc; // Skip if chatbot is not populated or missing _id
